...
#              Total reports
#              2017-01-23 - updated over the past 3 months to include additional
#              reports and ability to choose option to run all reports at once.
#
# To Do List/Future ideas:
# - have script pull from the feature service instead of geodatabase so that
#   everyone can run script
# - any other reports to include?
#
#-------------------------------------------------------------------------------
print "Welcome to FIND Data Mangement Report Creator. You will be prompted to choose from the following reports:"
print "DM Total - This report will return a summary of all records currently in the FIND database"
print "DM Ready - This report will return all records in the FIND database that are ready to be processed by the data management team"
print "DM Pending - This report will return all records that are listed as DM Pending"
print "DM Biologist - This report will return a separate report for each biologist that includes the status of all records belonging to them"
print "DM Reviewer - This report will return a separate report for each ID reviewer that includes information about records ready for them to review"
...
input_features = ["FIND3.DBO.el_pt", "FIND3.DBO.el_line", "FIND3.DBO.comm_poly", "FIND3.DBO.comm_pt", "FIND3.DBO.el_poly", "FIND3.DBO.survey_poly"]
...
# Path of FIND enterprise geodatabase
elementGDB = r"Database Connections\\FIND3.Default.pgh-gis.sde"

# county layer
counties = r"W:\\LYRS\\Boundaries_Political\\County Hollow.lyr"

# folder where output reports will be saved
ReportsPath = r"P:\Conservation Programs\Natural Heritage Program\Data Management\Instructions, procedures and documentation\FIND\FIND_2016\Reports"
...
def countyinfo(elementGDB, counties):
    ...
    for input, output in zip(input_features, elementShapefiles):
        ...
        keepFields = ["OID", "COUNTY_NAM", "refcode", "created_by", "created_on", "dm_stat", "dm_stat_comm", "last_up_by", "last_up_on", "element_type", "created_by", "elem_name", "id_prob", "id_prob_comm", "specimen_taken", "specimen_count", "specimen_desc", "curatorial_meth", "specimen_repo", "voucher_photo"]
        for field in fieldmappings.fields:
            if field.name not in keepFields:
                fieldmappings.removeFieldMap(fieldmappings.findFieldMapIndex(field.name))

        # run the spatial join tool
        arcpy.SpatialJoin_analysis(target_features, counties, element_features, "#", "#", fieldmappings)

def elementType():
    '''function that adds field identifying the type of element'''
    for elementShapefile in elementShapefiles:
        input_feature = os.path.join(env.workspace, elementShapefile)

        arcpy.AddField_management(input_feature, "element_type", "TEXT", field_length = 15, field_alias = "Element Type")

        arcpy.CalculateField_management(input_feature, "element_type", "'" + elementShapefile + "'", "PYTHON_9.3")

def mergetables():
    '''function that merges all element tables'''

    # converts all element shapefiles to tables
    for input, output in zip(elementShapefiles, elementTables):
        input_feature = os.path.join(env.workspace, input)
        table = arcpy.TableToTable_conversion(input_feature, env.workspace, output)

    # merges all element tables
...
def manipulateTable(pivotTable):
    ...
    arcpy.JoinField_management(pivotTable, "refcode", join, "refcode", ["survey_site_dmstat", "dm_stat_comm"])

    join = os.path.join(env.workspace, "elementRecords")
    arcpy.JoinField_management(pivotTable, "refcode", join, "refcode", ["COUNTY_NAM", "created_by", "created_on", "last_up_by", "last_up_on", "element_type"])

    arcpy.AddField_management(pivotTable, "Location", "TEXT", "", "", 1, "Location", "", "", "")
    West = ["ERIE", "CRAWFORD", "MERCER", "LAWRENCE", "BEAVER", "WASHINGTON", "GREENE", "VENANGO", "BUTLER", "ALLEGHENY", "FAYETTE", "WESTMORELAND", "ARMSTORNG", "INDIANA", "CLARION", "JEFFERSON", "FOREST", "WARREN", "MCKEAN", "ELK", "CLEARFIELD", "CAMBRIA", "SOMERSET", "BEDFORD", "BLAIR", "CENTRE", "CLINTON", "POTTER", "CAMERON", "HUNTINGDON", "FULTON", "FRANKLIN"]
...
def dmready(pivotTable):
    ...
    with arcpy.da.UpdateCursor(pivotTableCOPY, ["dmready", "total_records", "survey_site_dmstat"]) as cursor:
        for row in cursor:
...
def dmbiologist(pivotTable):
    ...
    for ref, name in zip(refname, createnames):
        with arcpy.da.UpdateCursor(pivotTableCOPY, ["refcode", "created_by"]) as cursor:
            for row in cursor:
                if row[0] is None or row[1] is None:
                    pass
                else:
                    if (row[1].lower() == "arcgis" or row[1].lower() == "tjadmin" or row[1].lower() == "administrator" or row[1].lower() == "bgeorgic") and ref in row[0].lower():
                        row[1] = name
                        cursor.updateRow(row)

    outPath = "P:\\Conservation Programs\\Natural Heritage Program\\Data Management\\Instructions, procedures and documentation\\FIND\\FIND_2016\\Reports\\Biologist Status Reports"
...
def idreview(elementRecords):
    ...
    keepFields = ["OID", "COUNTY_NAM", "refcode", "created_by", "created_on", "dm_stat", "Reviewer", "dm_stat_comm", "last_up_by", "last_up_on", "element_type", "id_prob", "id_prob_comm", "specimen_taken", "specimen_count", "specimen_desc", "curatorial_meth", "specimen_repo", "voucher_photo", "SCIENTIFIC_NAME", "ELEMENT_CODE"]
...
reportType = raw_input("Which report would you like to run? (please enter 'DM Pending', 'DM Ready', 'DM Biologist', 'DM Reviewer', 'DM Total, or DM All')")
